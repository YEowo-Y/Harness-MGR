...
## 2026-06-09 — OFF-RAMP FINAL REVIEW + WRITE-GATE RELAXED

Owner brought the (c) not-before floor forward to today ("把计划时间提前到今天，我现在就要最终复核") and asked for a tool re-check against the freshly-updated Claude Code. Both done.

**CC re-check (2.1.170 on disk).** `claude --version` = **2.1.170** (npm-global), up from 2.1.168. NOTE: this SESSION runs in-memory as 2.1.168 (the on-disk update takes effect on the next CC launch), so the machine-countable gate_pass row records `cc_version` 2.1.168. The tool was re-checked against the CURRENT on-disk `~/.claude` surface (which already carries the new keys): `selftest --release-gate` **pass:true** (all 7 steps; doctor-smoke fixture 25 checks / 0 error), real-harness read-only `doctor` 25 checks / **0 ERROR** / 19 warn / 31 info (warns/infos are faithfully-reported facts, not regressions; warn 19 unchanged vs 2.1.168, info 29→31 = the new `scheduled-tasks` dir + keys), full suite **2539 / 0 fail**. Schema-canary drift vs the 2.1.168 baseline = additive `appKeys +[hasOpenedAgentsView, unpinFable5LaunchEffort] -[opus48LaunchSeenCount]` + `topDirs +[scheduled-tasks]` — additive app-state/runtime keys + one CC-retired internal counter (`opus48LaunchSeenCount`; the new `unpinFable5LaunchEffort` is the Fable-5 analog). **NO removed DIMENSION** (the format-break alarm) — discovery healthy on the new surface. Re-baselined → status `clean` / 0 changes. `--release-gate --log` appended a gate_pass row → tally **29 → 30**.

**Off-ramp conditions — ALL MET → review PASS.** (a) 6 reversible `--apply` reps / zero incidents (unchanged). (b) canary survived 2.1.160→2.1.168 AND re-checked clean for the 2.1.170 binary, no write regression. (c) floor brought to today by the owner.

**DECISION (owner, via AskUserQuestion): RELAX NOW** — chosen over keeping the belt or a snapshot-only middle.

**RELAXATION SHIPPED (this session).** `src/cli/write-gate.mjs::resolveWriteIntent` relaxed: `--apply` ALONE now enables a governed write; `CLAUDE_MGR_ENABLE_WRITES` is no longer required. It is now an explicit OPT-OUT LOCK — `=0` hard-disables writes (exit 3 `writes-disabled-env`); unset / `1` / any other value allows writes with `--apply`. Built via a build→review(security + migration lenses)→fix Workflow: SEPARATE **security-reviewer** (verified the ONLY `enableWrites:true` path is apply-truthy-AND-not-locked — no write without `--apply`; `=0` locks; never-throws; all 11 callers go through the single source) + **code-reviewer** (verified the 15→16 closed-gate test migrations retriggered via `env=0` with refusal coverage PRESERVED [43 `writes-disabled-env` assertions, zero dropped] + the relaxation positively tested) → **APPROVE 0 Blocker / 0 High / 0 Medium**. New `test/write-gate.test.mjs` (8 cases) + closed-gate tests migrated (env-unset → env=0) + relaxation-positive legs (dry-run-vs-apply: env-unset `--apply` restores byte-identical). 16 satellite "set =1" guidance strings/docstrings refreshed to the relaxed model (usage(), the 4 ops dry-run hints, lock/recover needs-apply errors, command-module docstrings). A pre-existing `cascade-command.mjs` CLI coverage gap (77→97% line, 45→91% branch), surfaced by the release-gate coverage step when the string-refresh dragged it into the changed-file set, was fixed LEGITIMATELY with a new `test/cli-cascade.test.mjs` (14 cases) — NOT by reverting the doc edit. Full suite **2539 / 0**; release-gate **pass:true**. 30 per-file commits pushed.

**⚠ Migration tradeoff (logged honestly):** the gate's DEFAULT changed — an UNSET env var now ALLOWS writes (with `--apply`) where it previously BLOCKED them. Any script/CI that relied on "unset → blocked" must now set `CLAUDE_MGR_ENABLE_WRITES=0` to keep that lock (surfaced in CHANGELOG.md migration notes). The latent-bug calendar soak was thin (~1 day vs the original 30) — accepted by the owner given the strong (a)+(b) evidence; the residual risk is short time-in-use, mitigated by the snapshot-reversibility of every write + the `=0` opt-out.
