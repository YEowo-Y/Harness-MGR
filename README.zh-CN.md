...
就很难看清到底装了什么、谁遮蔽了谁、某个改动是否安全。`claude-mgr` 给你这种可见性，
并让你的改动**可审计、可回滚**。
...
- **零网络**——`claude-mgr` 自身代码不发起任何网络请求，并由机器级检查强制保证
  （`selftest --boundary`）。
...
| 入口 | 怎么获得 | 示例 |
|------|---------|------|
| `claude-mgr`（在 PATH 上） | 在仓库里执行一次 `npm link` | `claude-mgr inventory` |
...
下文示例为简洁起见统一用 `claude-mgr`。

## 两个目标：Claude 与 Codex

`claude-mgr` 用一套工具、一套测试、一张安全网治理两个 harness——不是分叉。全局
`--target` 选择治理哪一个：

```sh
claude-mgr inventory --target claude    # ~/.claude（默认）
claude-mgr inventory --target codex     # ~/.codex
claude-mgr compare claude,codex         # 两个 harness 各有什么，并排对比
```
...
- **退出锁。** 设 `CLAUDE_MGR_ENABLE_WRITES=0` 可硬性禁用所有受管写入（比如在 CI 里）；
...
- **零网络。** `update` 和 `mcp remove` 通过沙箱化 spawn 委托给外部 `claude` CLI——
  网络行为（如果有）属于那个进程，不属于 `claude-mgr`。

完整威胁模型见 [`docs/threat-model.md`](docs/threat-model.md)。

## MCP 服务器

`claude-mgr` 可以把它的**只读**视图作为 Model Context Protocol 服务器、通过 **stdio**
...
```sh
npm install
claude mcp add claude-mgr -- node /你的仓库绝对路径/claude-mgr/src/mcp/server.mjs
```
...
（带 `CLAUDE_MGR_ENABLE_WRITES=0` 退出锁），并可通过自动快照与 `rollback` 回滚。
