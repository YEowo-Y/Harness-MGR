...
test('integration: checks array is [1,2,3,5,18,6,7,8,9,10,11,12,22,23,13,14,16,20,21,25,17,24,4,15,19] with #1 error + #2 warn', () => {
  const r = runDoctor({
    now: NOW,
  ...
  assert.deepEqual(r.checks.map((c) => c.id), [1, 2, 3, 5, 18, 6, 7, 8, 9, 10, 11, 12, 22, 23, 13, 14, 16, 20, 21, 25, 17, 24, 4, 15, 19]);
...
test('integration: CHECKS registry has 25 entries starting with ids 1 and 2', () => {
