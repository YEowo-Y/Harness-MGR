...
test('integration: checks array is [1,2,6,7,8,9,10,11] with #1 error + #2 warn', () => {
  const r = runDoctor({
    now: NOW,
  ...
  assert.deepEqual(r.checks.map((c) => c.id), [1, 2, 6, 7, 8, 9, 10, 11]);
...
test('integration: CHECKS registry has 8 entries starting with ids 1 and 2', () => {
