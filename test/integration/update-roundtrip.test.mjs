...
test('update-roundtrip: installed_plugins.json with token-shaped content is captured (skipSecretFilter)', async (t) => {
  // HEADLINE RED→GREEN oracle for the 2026-06-10 audit follow-up:
  // installed_plugins.json containing a ghp_-shaped token was SILENTLY DROPPED by
  // the secret filter before skipSecretFilter:true was added — making the update
  // delegation irreversible.  This test proves the snapshot manifest CAPTURES the
  // file even when its content triggers the content-sniff leg.
  //
  ...
  // A token-shaped value (ghp_ + exactly 36 alphanum chars) inside installed_plugins.json —
  // this triggers the content-sniff 'github-token' pattern in snapshot-secrets-filter.
  ...
  const installed = {
    ...
    plugins: {
      'demo-plugin@demo-mkt': [
        { name: 'demo-plugin', marketplace: 'demo-mkt', version: '1.0.0', scope: 'user',
          // embed the token as a comment-style field to trigger content-sniff
          _test_token: TOKEN },
      ],
    },
  };
  ...
  try {
    ...
    assert.ok(
      entry,
      'manifest MUST capture plugins/installed_plugins.json even when it contains a token-shaped value ' +
      '(requires skipSecretFilter:true — this is the reversibility fix oracle)',
    );
    assert.equal(
      entry.preSha256,
      sha256Hex(installedBytes),
