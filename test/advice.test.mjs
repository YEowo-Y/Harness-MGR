...
const GOLDEN_EXPECTED = {
  advice: [
    ...
    },
    {
      ...
      fix: 'Edit the file at the reported line and column until it is valid JSON, then verify the result with harness-mgr `config show-effective`.',
      fixZh: '在报告指出的行和列处编辑文件，直到它成为合法的 JSON，然后用 harness-mgr 的 `config show-effective` 核对结果。',
    ...
    },
    {
      ...
      fix: "Rename one copy, or delete the duplicate you don't want. harness-mgr `remove <kind>:<name>` (kind = skill/agent/command) is dry-run by default and snapshots before deleting, so it is reversible.",
      fixZh: '给其中一份改名，或删掉你不想要的那份重复项。harness-mgr 的 `remove <kind>:<name>`（kind 为 skill/agent/command，即技能/智能体/命令）默认只是预演，删除前会先做快照，所以可以撤销。',
