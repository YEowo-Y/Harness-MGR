...
test('registry: full id order is [1,2,3,5,18,6,7,8,9,10,11,12,22,23,13,14,16,20,21,25,17,24,26,27,28,29,4,15,19]', () => {
  const r = runDoctor({}, { activeProbes: true });
  assert.deepEqual(
    r.checks.map((c) => c.id),
    [1, 2, 3, 5, 18, 6, 7, 8, 9, 10, 11, 12, 22, 23, 13, 14, 16, 20, 21, 25, 17, 24, 26, 27, 28, 29, 4, 15, 19],
  );
});

test('registry: CHECKS length is now 29 (26 passive + 3 active)', () => {
  const r = runDoctor({}, { activeProbes: true });
  assert.equal(r.checks.length, 29);
});
