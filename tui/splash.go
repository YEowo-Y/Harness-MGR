...
// splashMascot is the corner pet's "eyes open" frame: a 5-line CAT FACE — pointy
// /\ ears over a rounded box head (╭ ╮ │ ╰ ╯) that encloses big ◉ eyes, a ▿
// nose, and a rounded ╰───╯ jaw — shown in the dashboard's top-right.
// renderMascot colors it with a vivid multi-stop gradient (mascotStops); the
// Update loop briefly swaps in splashMascotBlink to blink. Only shown when
// unicodeEnabled(). The header-height reservation (mascotExtraRows) is derived
// from len(splashMascot), so the row count is free to change without touching
// splitDims.
...
// the big eyes (◉ ◉) close to dashes (- -), so briefly swapping to it reads as a
