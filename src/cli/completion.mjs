...
export function renderBashCompletion(model) {
  ...
  return `# claude-mgr bash completion (generated by \`claude-mgr completion bash\`).
# Install (one-time): source <(claude-mgr completion bash)
# or append the output to ~/.bashrc.
_claude_mgr_complete() {
...
complete -F _claude_mgr_complete claude-mgr
`;
}
...
export function renderPwshCompletion(model) {
  ...
  return `# claude-mgr PowerShell completion (generated by \`claude-mgr completion powershell\`).
# Install (one-time): claude-mgr completion powershell | Out-String | Invoke-Expression
# or append the output to $PROFILE.
Register-ArgumentCompleter -Native -CommandName 'claude-mgr','claude-mgr.ps1' -ScriptBlock {
