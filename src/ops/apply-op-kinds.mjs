 ...
 * op, or null when it is valid. A create/overwrite needs a non-empty `target` AND a
 * string `content`; a delete/delete-dir needs a non-empty `target` and NO content; a
 * config-edit needs a non-empty `target`, a boolean `desired`, a `selector` object, and
 * NO `content` (it splices a single token, never whole-file content). Kind errors and
 * missing-field errors carry distinct codes. Pure.
 * @param {unknown} op
...
export function invalidOpReason(op) {
  ...
  if (!isWrite && !isDelete && !isDirDelete && !isConfigEdit) {
    return { code: 'apply-op-kind-unsupported',
      message: `apply supports only ${[...WRITABLE_KINDS, ...DELETABLE_KINDS, ...DIR_DELETABLE_KINDS, ...CONFIG_EDIT_KINDS].join('/')} ops` };
  }
